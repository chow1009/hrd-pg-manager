...
while ($true) {
    ...
    Start-Sleep -Seconds 5
                    ...
                    Add-Content -Path $gitignore -Value ([Environment]::NewLine + '.expo/' + [Environment]::NewLine)
                    Write-Host 'Added .expo/ to .gitignore for Expo Doctor compliance.' -ForegroundColor Green
                } else {
                    Write-Host '.expo/ is already ignored.' -ForegroundColor Green
                }

                Write-Host '--- 3. Short QA workspace ---' -ForegroundColor Yellow
